...
On startup, ASAN reserves a 16 TB region of virtual address space via `VirtualAlloc(MEM_RESERVE)`. No physical memory is committed until shadow pages are first written — Windows lazily commits 64 KB-aligned chunks on demand, keeping actual RAM usage proportional to what the application touches.
...
When `free()` is called, ASAN does not immediately return memory to the OS. Instead the freed block is:
...
Compiling user code with `/GH` and `/Gh` causes MSVC to inject calls to `_penter()` at the start and `_pexit()` at the end of every function. ASAN uses these to track per-thread call depth.
...
// Low-level — used internally by CHECK_READ / CHECK_WRITE
...
#define ASAN_ENABLE_MSVC_HOOKS      1         // Enable /GH /Gh call depth tracking
```
...
`test_clean` is expected to pass with zero errors. The four error tests (`heap_overflow`, `use_after_free`, `double_free`, `stack_overflow`) are marked `WILL_FAIL` in CMake — they are treated as passing when they `abort()`.

### Compiling User Code

```bat
cl /Zi /GH /Gh /Od /I path\to\include user_code.cpp my_asan.lib DbgHelp.lib /link /DEBUG
```

| Flag | Purpose |
|------|---------|
| `/Zi` | Generate full debug info (.pdb) for DbgHelp symbol resolution |
| `/GH` | Inject `_pexit()` call at every function exit |
| `/Gh` | Inject `_penter()` call at every function entry |
...
```
================================================================
  my_asan: ERROR: heap-buffer-overflow
================================================================
